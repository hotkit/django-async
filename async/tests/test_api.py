...
class TestRemoveOldJobs(TestCase):
    ...
    def test_remove__cancelled_jobs(self, mock_get_today_dt):
        ...
        j1.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              30)
        j1.save()

        j2.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              15)
        j2.save()

        j3.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              15)
        j3.save()
    ...
    def test_remove__mixing_jobs(self, mock_get_today_dt):
        ...
        j4.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              30)
        j4.save()

        j5.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              15)
        j5.save()

        j6.cancelled = get_d_before_dt_by_days(datetime.datetime(2014, 1, 31),
                                              15)
        j6.save()

        api.remove_old_jobs()
        jobs = Job.objects.filter(name__in=['job-1', 'job-2', 'job-3', 'job-4', 'job-5', 'job-6'])
